...
for (const deck of decks) {
  ...
  if (html.includes('data-background-image="assets/slides/')) {
    throw new Error(`${deck.name} should not use rendered PowerPoint slides as backgrounds; recreate slides as editable markup`);
  }

  if (!html.includes('class="slide-subtitle"') && !html.includes("class='slide-subtitle'")) {
    throw new Error(`${deck.name} should include semantic blue slide subheadings with class=\"slide-subtitle\"`);
  }
